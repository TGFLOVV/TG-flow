...
export default function Settings() {
  ...
  const sendVerificationCodeMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      // Специфичные ошибки email
      if (errorMessage.includes("уже привязан к другому аккаунту") || 
          errorMessage.includes("уже используется") || 
          errorMessage.includes("already in use") ||
          error?.response?.data?.errorCode === "EMAIL_ALREADY_EXISTS") {
        errorMessage = "Данная почта уже занята другим пользователем";
      } else if (errorMessage.includes("неверный") || errorMessage.includes("invalid")) {
        errorMessage = "Некорректный email адрес";
      } else if (errorMessage.includes("не найден") || errorMessage.includes("not found")) {
        errorMessage = "Email сервис недоступен";
      } else if (error?.response?.status === 409) {
        errorMessage = "Данная почта уже занята другим пользователем";
      } else if (error?.response?.status === 400 && 
                 error?.response?.data?.errorCode === "EMAIL_ALREADY_EXISTS") {
      ...
      } else if (error?.response?.data?.errorCode === "EMAIL_SEND_FAILED") {
        errorMessage = "Не удалось отправить код на email. Проверьте правильность адреса";
      } else if (errorMessage === "Произошла неожиданная ошибка") {
        errorMessage = "Не удалось отправить код верификации";
      }
  ...
  const verifyEmailCodeMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setVerificationCode("");
      setPendingEmail("");
      toast({ title: "Email подтвержден", "Ваш email успешно подтвержден и обновлен" });
    },
    onError: (error: any) => {
      console.error("Error verifying email code:", error);
      let errorMessage = extractErrorMessage(error);
      
      // Специфичные ошибки верификации
      if (errorMessage.includes("истек") || errorMessage.includes("expired")) {
        errorMessage = "Код верификации истек. Запросите новый код";
      } else if (errorMessage.includes("неверный") || errorMessage.includes("invalid")) {
        errorMessage = "Неверный код верификации";
      } else if (errorMessage.includes("не найден") || errorMessage.includes("not found")) {
        errorMessage = "Код не найден. Запросите новый код";
      } else if (error?.response?.status === 400) {
        errorMessage = "Неверный код верификации";
      ...
      toast({ title: "Ошибка верификации", errorMessage, variant: "destructive" });
    },
  });
